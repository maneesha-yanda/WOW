
<?php 

/*session_start();


     $con = mysqli_connect("localhost","root","","ecommerce");

     $email=$_POST['email'];
     $password=$_POST['password'];
     
    
    $email_select_query ="select * from users where Email='$email' && Password='$password'";
      $result= mysqli_query($con,$email_select_query);
     $num=mysqli_num_rows($result);

     if($num==1)
     { 
     
     }

     else
     {
        header("location:2.login.php?message=email and passowrd not valid");
      
     }
    
    */


?>


<!DOCTYPE html>
<html lang="en">
<head>
     <!--adding bootstrap files-->
            <!--bootstrap css file-->
            <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
            <!--jQuery library-->
            <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
            <!--Latest compiled and minified JavaScript-->
            <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
    <!--css link     (we added javascript link at the last)-->    

    <!-- font awesome icon link (for icons) -->
    <link href="https://maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css" rel="stylesheet" integrity="sha384-T8Gy5hrqNKT+hzMclPo118YTQO6cYprQmhrYwIiQ/3axmI1hQomh7Ud2hPOy8SP1" crossorigin="anonymous">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>home page</title>
   
    <style>
body
{
width:100%;
height:auto;
}






/* for watches  */

.w     /* for border */
{
    width:100%;
    height:auto;
    border: 1px solid;
    border-radius: 5px;
    margin-top:20px;
   
}

.watch
{
    background-color: rgb(233, 233, 241);
    width:100%;
    padding: 7px;
    float: left;
    font-size:15px;
}

#img1 img
{
    width:255px;
}

.w1
{
    margin-top: 20px;
} 
.h3
{
    text-align: center;
    font-size: 20px;
}
.b
{
background-color:#266188;
margin:15px;
padding: 7px;
color: white;
border-radius: 5px;
text-align: center;
}
.b:hover{
   
   background-color:  #173e57;
    color:rgb(253, 249, 249);
    cursor: pointer;

}

#d4
{
    margin-top: 20px;
}
.ma
{
    margin-bottom: 20px;
}




/*for watch icon   */

#ic
{
    width:25px;
    position:absolute;
    margin-top:-24px;
    margin-left:50px;
}
    </style>



        
</head>
<body>




  <div id="main">  
            <!--for navbar-->
            <nav class="navbar navbar-default">
                <div class="container">
                    <div class="navbar-header">
                        <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#mynavbar">
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                        </button>

                        <div id="d2" class="navbar-brand">WOW<img id="ic" src="https://image.flaticon.com/icons/png/128/4305/4305522.png"></div>
                    </div>
                    <div class="collapse navbar-collapse" id="mynavbar">
                        <ul class="nav navbar-nav navbar-right">
                        <li><a href="#"><i class="fa fa-shopping-cart">&nbsp;</i> Cart</a></li> 
                            <li><a href="7.settings page.php" id="login"><i class="fa fa-user">&nbsp;</i> Settings</a></li> 
                            <li><a href="#"><i class="fa fa-sign-out">&nbsp;</i> Logout</a></li> 
                            
                        </ul>
                    

                    </div>
                </div>
            </nav>
    

            <!--for watches(body)-->
            <div id="d4">
                    <div class="col-sm-4 ma">
                        <div id="d5" class="w">
                            <div  class="watch">#1</div>
                            <div  class="col-sm-12 w1" id="img1">
                                <img src="https://cdn.shopify.com/s/files/1/1245/9327/products/97N101_Bulova_Little_Switzerland_large.png?v=1571439090" class="container-fluid img-responsive">
                            </div>
                            <h3 class="h3">Price:1400/-</h3>
                           <a href="cart.php"> <div class="b">Add to cart</div></a> 
                        </div>
                    </div>
                   <div class="col-sm-4 ma">
                        <div id="d6" class="w">
                            <div  class="watch">#2</div>
                            <div  class="col-sm-12 w1">
                                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnIr546AxrERemq4Qq91KOgu7o17KROvDAFA&usqp=CAU"  class="container-fluid img-responsive">
                            </div>
                            <h3 class="h3">Price:1600/-</h3>
                            <a href="cart.php">  <div class="b">Add to cart</div> </a> 
                        </div>
                    </div>
                    <div class="col-sm-4 ma">
                        <div id="d7" class="w">
                            <div  class="watch">#3</div>
                            <div  class="col-sm-12 w1">
                                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRSxjPnYdHoM5IGP8F2k3OtXcIYdyvbEDvXoQ&usqp=CAU" alt="image"  class="container-fluid img-responsive">
                            </div>
                            <h3 class="h3">Price:1850/-</h3>
                            <a href="cart.php">  <div class="b">Add to cart</div> </a> 
                        </div>
                    </div>
                    <div class="col-sm-4 ma">
                        <div id="d8" class=" w">
                            <div  class="watch">#4</div>
                            <div  class="col-sm-12 w1"> <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTGA3QRPYgz6tjOQyGDYejvEYHs5CA-WZCSQg&usqp=CAU"  class="container-fluid img-responsive"> 
                            </div>
                            <h3 class="h3">Price:1210/-</h3>
                            <a href="cart.php"> <div class="b">Add to cart</div> </a> 
                        </div>
                    </div>
                    <div class="col-sm-4 ma">
                        <div id="d9" class=" w">
                            <div  class="watch">#5</div>
                            <div  class="col-sm-12 w1">
                            <img src="data:image/jpeg;base64,"  class="container-fluid img-responsive">
                            </div>
                            <h3 class="h3">Price:1500/-</h3>
                            <a href="cart.php">   <div class="b">Add to cart</div> </a> 
                        </div>
                    </div>
                    <div class="col-sm-4 ma">
                        <div id="d10" class=" w">
                            <div  class="watch">#6</div>
                            <div  class="col-sm-12 w1">
                                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOLFFCS7L2feWkfJ9FxqVbWUUXZ4u0zAFBNg&usqp=CAU" alt="image"  class="container-fluid img-responsive">
                            </div>
                            <h3 class="h3">Price:1000/-</h3>
                            <a href="cart.php">    <div class="b">Add to cart</div> </a> 
                        </div>
                    </div>  
                        
            </div>
            <br>


      

   
    

</body>
</html>